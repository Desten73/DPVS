...
public class TextArchitect
{
    ...
    // текст в игровом пронстранстве
    private TextMeshPro tmpro_world;

    // выбор нужного варианта для размещения текста
    public TMP_Text tmpro => tmpro_ui != null ? tmpro_ui : tmpro_world;

    public string currentText => tmpro.text;
    public string targetText { get; private set; } = "";
    // то, что уже есть в архитектора. Нужно для добавления текста к прошлому тексту
    public string preText { get; private set; } = "";
    private int preTextLength = 0;

    public string fullTargetText => preText + targetText;

    // 3 метода. Мгновенный, пишущая машинка, сокрытие
    public enum BuildMethod { instant, typewriter, fade}
    public BuildMethod buildMethod = BuildMethod.typewriter;

    public Color textColor { get { return tmpro.color; } set { tmpro.color = value; } }

    // возвращает базовую скорость * коэфициент(настраивается пользователем). set изменяет коэффициент скорости
    public float speed { get { return baseSpeed * speedMultiplier; } set { speedMultiplier = value; } }
    private const float baseSpeed = 1;
    public float speedMultiplier = 1;

    // скорость появление букв в цикл
    ...
    // добавление строки к прошлой строке
    ...
    IEnumerator Building()
    {
        ...
        switch(buildMethod)
        {
            case BuildMethod.typewriter:
                yield return Build_Typewritter();
                break;
    ...
    // процесс сборки завершен, обнуляем переменные
    ...
    // подготовка, в зависимости от типа отображения текста
    ...
    private IEnumerator Build_Typewritter()
    {
