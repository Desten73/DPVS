...
namespace CHARACTERS
{
    public class CharacterManager : MonoBehaviour
    {
        ...
        public Character CreateCharacter(string characterName)
        {
            if (characters.ContainsKey(characterName.ToLower()))
            {
                Debug.LogWarning($"Персонаж уже '{characterName}' создан. Невозможно создать персонажа с таким же именем.");
                return null;
            }
        ...
        public void SortCharacters(string[] characterNames)
        {
            ...
            int startingPririty = remainingCharacters.Count > 0 ? remainingCharacters.Max(c => c.priority) : 0;
            for (int i = 0; i < sortedCharacters.Count; i++)
            {
                Character character = sortedCharacters[i];
                character.SetPriority(startingPririty + i + 1, autoSortCharactersOnUI: false);
            }
