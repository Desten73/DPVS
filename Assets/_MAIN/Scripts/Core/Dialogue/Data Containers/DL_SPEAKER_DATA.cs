...
namespace DIALOGUE
{
    public class DL_SPEAKER_DATA
    {
        ...
        /// <summary>
        /// Отображение имени, которое задано. Если есть псевдоним - берем его, иначе - берем настоящее имя
        /// </summary>
        public string dispayName => castName != string.Empty ? castName : name;
        ...
        public DL_SPEAKER_DATA(string rawSpeaker)
        {
            ...
            // Заполнение данные для избежания ошибки нулевой ссылки
            castName = "";
            castPosition = Vector2.zero;
            CastExpressions = new List<(int layer, string expression)>();

            // Если нет совпадений значит нам пришло только имя говорящего
            if (matches.Count == 0)
            {
                name = rawSpeaker;
                return;
            ...
            // Иначе отделяем имя говорящего от остальной информации
