...
namespace DIALOGUE
{
    public class DL_DIALOGUE_DATA
    {
        ...
        {
            segments = RipSegmants(rawDialogue);
        }

        private List<DIALOGUE_SEGMENT> RipSegmants(string rawDialogue)
        {
            ...
            int lastIndex = 0;
            // поиск первого или единственного сегмента в файле
            ...
            for (int i = 0; i < matches.Count; i++)
            {
                ...
                // получаем стартовый сигнал для сегмента
                string signalMatch = match.Value; //{A}
                signalMatch = signalMatch.Substring(1, match.Length - 2);
                string[] signalSplit = signalMatch.Split(' ');

                segment.startSignal = (DIALOGUE_SEGMENT.StartSignal)
                    Enum.Parse(typeof(DIALOGUE_SEGMENT.StartSignal), signalSplit[0].ToUpper());

                // получение задержки сигнала
                if (signalSplit.Length > 1)
                    float.TryParse(signalSplit[1], out segment.signalDelay);

                // получение диалога для сегмента
        ...
        public struct DIALOGUE_SEGMENT
        {
            ...
            // C - clear, A - append, WA - wait append, WC - wait clear
